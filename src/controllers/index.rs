...
use crate::config::{self, LOGIN_ERROR_MAX, LOGIN_LOCKED_TIME};
use crate::models::{Admins, Index as ThisModel, OSSData, OSSResult, UploadResult};
use actix_multipart::Multipart;
use actix_session::Session;
use actix_web::{web, web::Form, HttpRequest, HttpResponse};
use fluffy::{datetime, db, model::Model, random, response, tmpl::Tpl, utils};
...
impl Index {
    ...
    pub async fn error(request: HttpRequest, tpl: Tpl) -> HttpResponse {
        ...
        let duration = if let Some(v) = queries.get(&"duration") {
    ...
    pub async fn login(session: Session, post: Form<HashMap<String, String>>) -> HttpResponse {
        ...
        if let Ok(failure) = session.get::<usize>("failure_count") {
            //检测登录失败次数
            if let Some(n) = failure {
                failure_count = n; //已经失败的次数
                if n > LOGIN_ERROR_MAX {
                    if let Err(message) =
                        session.set::<usize>("locked_time", datetime::timestamp() as usize)
                    {
        ...
        if let Some(row) = Admins::fetch_row(&mut conn, &query, Some(&cond)) {
            let (id, password, secret, login_count, role_id): (
                usize,
                String,
                String,
                usize,
            ...
            if password_enc != password {
                //对比加密之后的密码是否一致
                session
                    .set::<usize>("failure_count", failure_count + 1)
                    .unwrap();
    ...
    pub async fn change_pwd_save(
        ...
        if let Err(message) = ThisModel::check_change_pwd(&post) {
            //检测密码输入是否正确
            return response::error(message);
        }
        ...
        let mut conn = db::get_conn();
        let row = if let Some(r) = Admins::fetch_row(&mut conn, &query, Some(&cond)) {
            r
        } else {
            return response::error("检测用户信息失败");
        };
    ...
    pub async fn manage(session: Session, tpl: Tpl) -> HttpResponse {
        ...
        let mut user_name = "admin".to_owned();
        if let Ok(v) = session.get::<String>("user_name") {
            if let Some(n) = v {
                user_name = n;
            }
        }
    ...
    pub async fn right(session: Session, tpl: Tpl) -> HttpResponse {
        ...
        // cpu信息
        let cpu_num = if let Ok(n) = sys_info::cpu_num() {
            n
        } else {
            0
        };
        ...
        // cpu频率
        let cpu_speed = if let Ok(n) = sys_info::cpu_speed() {
            n
        } else {
            0
        };
        ...
        // 硬盘信息
        let disk_info = if let Ok(v) = sys_info::disk_info() {
            v
        } else {
        ...
        // 主机名称
        let host_name = if let Ok(v) = sys_info::hostname() {
            v
        } else {
            "".to_owned()
        };
        ...
        // 内存信息
        let mem_info = if let Ok(v) = sys_info::mem_info() {
            (v.total, v.free, v.avail)
        } else {
            (0, 0, 0)
        };
        ...
        // 操作系统
        let os_type = if let Ok(v) = sys_info::os_type() {
            v
        } else {
            "".to_owned()
        };
        ...
        // 操作系统版本
        let os_version = if let Ok(v) = sys_info::os_release() {
            v
        } else {
            "".to_owned()
        };
        ...
        // 进程数量
        let process_count = if let Ok(n) = sys_info::proc_total() {
            n
        } else {
            0
        };
        ...
        // 负载
        let avg = if let Ok(v) = sys_info::loadavg() {
            (v.one, v.five, v.fifteen)
        } else {
            (0., 0., 0.)
        };
    ...
    async fn upload_files(file_types: &[&str], mut payload: Multipart) -> HttpResponse {
        ...
        let upload_error = |code: u32, message: &str| {
            //上传成功返回
            upload_result(code, message, "")
        };
        let upload_success = |path: &str| {
            //上传失败返回
            upload_result(0, "", path)
        };
        while let Some(item) = payload.next().await {
            let mut field = if let Ok(v) = item {
                v
            } else {
                return upload_error(401, "获取上传文件失败");
            };
            ...
            let mut f = if let Ok(v) = web::block(|| std::fs::File::create(save_file_path)).await {
                v
            } else {
                return upload_error(405, "创建临时文件失败");
            };
            while let Some(chunk) = field.next().await {
                let data = if let Ok(v) = chunk {
                    v
                } else {
                    return upload_error(406, "读取文件信息失败");
                };
                f = if let Ok(v) = web::block(move || f.write_all(&data).map(|_| f)).await {
                    v
                } else {
                    return upload_error(408, "保存文件信息失败");
                };
            }
